...
Protected Class Main
	#tag Method, Flags = &h0
		Sub Constructor(myM As Double, myδ As Double, myf0 As Double, myR As Double, myβ As Double, myψangle As Double, myλ0 As Double, myΘ As Double, myΦ As Double, χ10 As Vector, χ20 As Vector, myPNOrder As Integer, myDet As Integer, mydτ0 As Double, myK As Double)
		  ...
		  // all necessary parameters, and creates an instance of the Evolver class to handle the phase evolution. Then, it performs steps
		  // of constant size, requesting values from the Evolver class after each step. Also at each step, it handles the waveform amplitude
		  ...
		  myEvolver = New Evolver(M, δ, f0, R, β, ψangle, λ0, Θ, Φ, χ1, χ2, dτ0)      //Create the new instance of the evolver class, giving it the necessary parameters from initial conditions
		  ...
		  dχ1ℓdχ10x = myEvolver.dχ1ℓdχ10x
		  dχ1ℓdχ10y = myEvolver.dχ1ℓdχ10y
		  dχ1ℓdχ10z = myEvolver.dχ1ℓdχ10z
		  dχ1ℓdχ20x = myEvolver.dχ1ℓdχ20x
		  dχ1ℓdχ20y = myEvolver.dχ1ℓdχ20y
		  dχ1ℓdχ20z = myEvolver.dχ1ℓdχ20z
		  
		  dχ2ℓdχ10x = myEvolver.dχ2ℓdχ10x
		  dχ2ℓdχ10y = myEvolver.dχ2ℓdχ10y
		  dχ2ℓdχ10z = myEvolver.dχ2ℓdχ10z
		  dχ2ℓdχ20x = myEvolver.dχ2ℓdχ20x
		  dχ2ℓdχ20y = myEvolver.dχ2ℓdχ20y
		  dχ2ℓdχ20z = myEvolver.dχ2ℓdχ20z
	...
	#tag Method, Flags = &h0
		Sub DetectorFunctions(hdrvtvs() As Double)
		  ...
		  For k As Integer = 1 to nDetectors
		    ...
		    Var c4α2λ2Φ As Double = Cos(4*αDet - 2*ΦDet)
		    ...
		    Var st4 as Double = -9*c2λ2Φ + c4α2λ2Φ
		    ...
		    Var dc As Double = (f2/16)*cosΘ*(9*c2λ2Φ - c4α2λ2Φ) - (3/8)*sinΘ*(c3α2λ1Φ + 3*c1α2λ1Φ)
	...
	#tag Method, Flags = &h0
		Sub DoMainStep()
		  ...
		  GetSpinValues  // get the values from Evolver
	...
	#tag Method, Flags = &h0
		Sub GetSpinValues()
		  // This method is called when we need the phase values from the Evolver class. It calls Evolver's ReturnValues function and
		  // stores these returned values to properties.
		  
		  Var spinValues() As Double = myEvolver.ReturnValues(τMain)
