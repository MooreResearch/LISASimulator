...
Protected Class Evolver
	...
	#tag Method, Flags = &h0
		Sub Constructor(myM As Double, myδ As Double, myf0 As Double, myR As Double, myβ As Double, myψangle As Double, myλ0 As Double, myΘ As Double, myΦ As Double, χ10 As Vector, χ20 As Vector, mydτ0 As Double)
		  // The Evolver class is meant to perform all steps to evolve the gravitational wave from the source to the detector, where it then
	...
	#tag Method, Flags = &h0
		Sub DoStep(dτ As Double)
		  ...
		  If Not (noSpin1 and noSpin2) Then 
		    mainSpinEvolver.SpinDoStep(dτP, dτ, vN)         //Evolve the main spin evolver
		  ...
		  // Set up the side spin evolvers
		  sideSpinEvolver1.SpinDoStep(dτP, dτ, vN)                                                          //Evolve the first side evolver that adds a little to χ1x
		  sideSpinEvolver2.SpinDoStep(dτP, dτ, vN)                                                         //Evolve the second side evolver that subtracts a little from χ1x
		  ...
		  Var αdotN As Double
		  If Not (noSpin1 and noSpin2) Then 
		    αdotN = mainSpinEvolver.αdotN         //Pull the value of αdot from the main spin evolver. NOTE: this value is calculated in the "present" and not the "future"
		  End 
		  ...
		  //Update value of Evolver's "clock"
		  τSpin = τSpin + dτ
	...
	#tag Method, Flags = &h0
		Function FindIdealdτ(init As Boolean = False) As Double
		  ...
		  // Then, the Evolver constructor calls this method to get this value.
	...
	#tag Method, Flags = &h0
		Sub InitDvdv0()
		  // This method is called by the Evolver constructor, and it initializes dv/dv0 by setting up the first two values.
	...
	#tag Method, Flags = &h0
		Sub InitDvdδ()
		  // This method is called by the Evolver constructor, and it initializes dvdδ by setting up the first two values.
	...
	#tag Method, Flags = &h0
		Sub InitDvdχ1ℓ()
		  ...
		  // This method is called by the Evolver constructor, and it initializes dvdχ1ℓ by setting up the first two values.
	...
	#tag Method, Flags = &h0
		Sub InitDvdχ2ℓ()
		  ...
		  // This method is called by the Evolver constructor, and it initializes dvdχ2ℓ by setting up the first two values.
	...
	#tag Method, Flags = &h0
		Sub InitSpinVariables()
		  ...
		  // to the Main Window when this method is called in the Evolver constructor, except for a few which are used to initialize the phase in 
		  ...
		  //Set up the main spin evolver and pull the values calculated in its constructor (the first two values for each parameter)
		  ...
		  //Set up the twelve side spin evolvers and use the values calculated to compute the first two values of each derivative with the centered difference equation
		  sideSpinEvolver1 = New SpinEvolver(v0, dτ0, δ, Ln0, ι0, α0, x1 + Δχ, y1, z1, x2, y2, z2)      //Set up one side evolver with a little added to x1
		  sideSpinEvolver2 = New SpinEvolver(v0, dτ0, δ, Ln0, ι0, α0, x1 - Δχ, y1, z1, x2, y2, z2)      //Set up the other side evolver with a little subtracted from x1
	...
	#tag Method, Flags = &h0
		Sub InitV()
		  // This method is called by the Evolver constructor, and it initializes v by setting up the first two values.
	...
	#tag Method, Flags = &h0
		Sub InitψAndDerivs()
		  ...
		  // difference approximation using the initial value of each phase or derivative. Then, this method is called in the Evolver
		  ...
		  Var αdot0 As Double
		  If Not (noSpin1 and noSpin2) Then
		    αdot0 = mainSpinEvolver.αdot0       //Pull the initial value of αdot from the main spin evolver
		  End If
	...
	#tag Method, Flags = &h0
		Function ReturnValues(τMain As Double) As Double()
		  // This method is how the Evolver class and Main class communicate with each other. The Main class calls this method, passing
		  // in a current time, and the Evolver class evolves until that time or interpolates at that time to return the appropriate values. This 
		  // method is also where the Evolver class is calling functions to calculate ideal time steps and actually calling the DoStep function.
